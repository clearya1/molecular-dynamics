...
void parallel_enforce_boundary(std::vector<Atom>& atoms, double L, double R, int sqnproc, std::string boundary_type) {
  ...
  /*if (rank==0) {
    cout << "boundary function" << std::endl;
  }*/
...
void skin_forces_indices_atoms(const std::vector<int> &cell_skin_ind, const std::vector<Atom> &cell_atoms, const std::vector<Atom> &neighbour_atoms, std::vector<std::vector<double> > &cell_forces, double &temp_potential, int send_rank)
{

  std::vector<double> forces_temp(3);
  double r;
  double neigh_fx, neigh_fy;
  std::vector<double> neigh_forces_x(neighbour_atoms.size());
  std::vector<double> neigh_forces_y(neighbour_atoms.size());

  for (int j = 0; j < neighbour_atoms.size(); j++)
  {
    neigh_fx = 0;
    neigh_fy = 0;
    for (int i = 0; i < cell_skin_ind.size(); i++) // loop over atoms within the cell..
    {
        ...
        if (r < R)
        {
          ...
          // add up the forces between the atoms in the skin (loop over i) and atom j (with opposite sign)
          neigh_fx -= forces_temp[1];
          neigh_fy -= forces_temp[2];
        }
      }
      // write the summed up forces into the j-th place (force corr. to atom j)
      neigh_forces_x[j] = neigh_fx;
      neigh_forces_y[j] = neigh_fy;
    }
    // all forces on neighbour atoms collected into 2 vectors (x,y directions)

    // send the force vectors back to the cell we received the atoms from
    MPI_Send(&(neigh_forces_x[0]), neighbour_atoms.size(), MPI_DOUBLE, send_rank, 0, comm);
    MPI_Send(&(neigh_forces_y[0]), neighbour_atoms.size(), MPI_DOUBLE, send_rank, 1, comm);

  }

void receive_forces(int n_send, int neigh_rank, std::vector<int> indices, std::vector<std::vector<double> > &cell_forces)
{
  std::vector<double> temp_x;
  std::vector<double> temp_y;
  // the length of the received force vector is the same as the amount of atoms we sent into the corr. direction
  temp_x.resize(n_send);
  temp_y.resize(n_send);
  for (int i = 0; i < n_send; i++)
  {
    temp_x[i] = 0.0;
    temp_y[i] = 0.0;
  }
  MPI_Recv(&(temp_x[0]), n_send, MPI_DOUBLE, neigh_rank, 0, comm, MPI_STATUS_IGNORE);
  MPI_Recv(&(temp_y[0]), n_send, MPI_DOUBLE, neigh_rank, 1, comm, MPI_STATUS_IGNORE);
  for (int i = 0; i < n_send; i++)
  {
    cell_forces[0][indices[i]] += temp_x[i];
    cell_forces[1][indices[i]] += temp_y[i];
  }
...
std::vector<std::vector<double> > update_force(const std::vector<Atom> &atoms, std::vector<double> &cell_energy, double R, double M, double sqnproc)
{
  ...
  // skin on at the top and right side of the cell
  // each cell sends its top, right and diagonal skin layer (3 directions)
  std::vector<Atom> recv_left;
  ...
  std::vector<int> ind_left; // indices of atoms in the skin layers
  ...
  int n_recv_left, n_recv_down, n_recv_downleft, n_recv_downright;
  int n_send_right, n_send_up, n_send_upright, n_send_upleft;

  // get the indices for the atoms located in all 4 skin layers in the cell + 4 corners
  for (int i = 0; i < num_atoms; i++)
  {
  ...
  //-------------------------------------- HORIZONTAL (send to the right and upright) --------------------------------------
  ...
  if (col%2==0) {   // send right from even columns first then from odd columns
  ...
  //------------------------------ VERTICAL (send in the up and upleft direction) ---------------------------------------------
  
  // now communicate the number of atoms that will be sent across horizontal borders
  
  if (row%2==0) {   // send up from even row first then from odd rows
    // Up
    ...
    if (row % 2 == 0)
    { // send up from even rows first then from odd rows. Then send down
  ...
  // now edit received atoms for processes on border
  ...
  // specify ranks to where send forces, D=down, DR=downright, DL=downleft, L=left
  neigh_rank_D = modulo(row - 1, sqnproc) * sqnproc + col;
  neigh_rank_DR = modulo(row - 1, sqnproc) * sqnproc + modulo(col + 1, sqnproc);
  neigh_rank_DL = modulo(row - 1, sqnproc) * sqnproc + modulo(col - 1, sqnproc);
  neigh_rank_L = row * sqnproc + modulo(col - 1, sqnproc);
  
  // specify ranks from which receive forces, R=right, UR=upright, U=up, UL=upleft
  neigh_rank_R = row * sqnproc + modulo(col + 1, sqnproc);
  neigh_rank_UR = modulo(row + 1, sqnproc) * sqnproc + modulo(col + 1, sqnproc);
  neigh_rank_U = modulo(row + 1, sqnproc) * sqnproc + col;
  neigh_rank_UL = modulo(row + 1, sqnproc) * sqnproc + modulo(col - 1, sqnproc);


  // use the skin_forces_indices_atoms function to loop over corresponding skins and 
  // compute forces on the left, downleft, down and downright
  // compute forces with atoms received from the downright process, send them back to the downright process etc
  skin_forces_indices_atoms(ind_downright, atoms, recv_downright, cell_forces, temp_potential, neigh_rank_DR);
  skin_forces_indices_atoms(ind_down, atoms, recv_down, cell_forces, temp_potential, neigh_rank_D);
  skin_forces_indices_atoms(ind_downleft, atoms, recv_downleft, cell_forces, temp_potential, neigh_rank_DL);
  skin_forces_indices_atoms(ind_left, atoms, recv_left, cell_forces, temp_potential, neigh_rank_L);
  
  // receive from 4 directions:
  // from upleft: receive force of length being the number of atoms we sent to upleft direction, from process UL
  receive_forces(n_send_upleft, neigh_rank_UL, ind_upleft, cell_forces);
  // from up
  receive_forces(n_send_up, neigh_rank_U, ind_up, cell_forces);
  // from upright
  receive_forces(n_send_upright, neigh_rank_UR, ind_upright, cell_forces);
  // from right
  receive_forces(n_send_right, neigh_rank_R, ind_right, cell_forces);
...
int main()
{
  ...
  int cell_size = 8;                   // Number of atoms per side of a cell
  ...
  if (rank==0) {
    if (write_pos=="True") {
      myfile_pos.open ("pos_par.dat");
      myfile_vel.open ("vel_par.dat");
    }
